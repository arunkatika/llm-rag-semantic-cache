...
from scripts.count_tokens import count_tokens, tokenizer
...
def process_document(uploaded_file: UploadedFile) -> list[Document]:
    ...
    # Save uploaded file to a temp location and CLOSE it before reading
    ...
    # After loading, safely delete the temp file
    try:
        os.unlink(temp_path)
...
def add_to_vector_collection(all_splits: list[Document], file_name: str):
    ...
    for idx, split in enumerate(all_splits):
        ...
        print(f"Chunk {idx} | Tokens: {token_count}")  # 💡 View in terminal
    ...
    if token_count > 800:
        print(f"⚠️ Warning: Chunk {idx} is unusually long ({token_count} tokens)")
    else:
